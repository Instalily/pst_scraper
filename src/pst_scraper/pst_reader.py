...
def read_folder_emails(pst: PersonalStorage, folder: FolderInfo, output_emails_path, output_attachments_path) -> tuple[int, int]:
    num_emails = 0
    num_attachments = 0

    for sub_folder in folder.get_sub_folders():
        added_emails, added_attachments = read_folder_emails(sub_folder,  output_emails_path, output_attachments_path)
...
def read_pst(pst_file_path: str, output_emails_path: str, output_attachments_path: str) -> tuple[int, int]:
    ...
    return read_folder_emails(pst, pst_root, output_emails_path, output_attachments_path)